...
def empty_output_file():
	...
	'''
	with open('expt3_output.txt', 'w') as f:
		pass
...
if __name__ == '__main__':
	...
	for i in range(100, 510, 50):
		generate_input("matrix.txt", i, i)
		generate_input("vector.txt", i, 1)	
		for j in ['1','2','3','4']:
			subprocess.run(["mpirun", "-use-hwthread-cpus", "-n", j, "expt3", "0"])
	
	# Get output from file
	with open("expt3_output.txt", 'r') as f:
		data = f.readlines()
	...
	plt.plot(X[::4], Y[::4], label='4')
	plt.plot(X[1::4], Y[1::4], label='3')
	plt.plot(X[2::4], Y[2::4], label='2')
	plt.plot(X[3::4], Y[3::4], label='1')
	plt.legend()
